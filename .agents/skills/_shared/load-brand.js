 ...
 * 1. DECK_BRAND_DIR env (absolute, or relative to repo root) — e.g. brand/cursor for field-kit
 * 2. <repoRoot>/brand/brand-pack.json
 *
 * Repo root is found by walking up from the deck JSON until brand/brand-pack.json
 * or (package.json + content-blocks/) exists — so exports under examples/assembled-
 * still find the dogfood brand pack.
 *
...
/**
 * Walk up from the deck JSON directory to the deck-machine repo root.
 * @param {string} inputAbs Absolute path to deck-content.json (or any file under the repo)
 */
function repoRootFromInput(inputAbs) {
  let dir = path.dirname(path.resolve(inputAbs));
  for (;;) {
    const brandPack = path.join(dir, 'brand', 'brand-pack.json');
    const dogfoodMarker =
      fs.existsSync(path.join(dir, 'package.json')) &&
      fs.existsSync(path.join(dir, 'content-blocks'));
    if (fs.existsSync(brandPack) || dogfoodMarker) {
      return dir;
    }
...
/**
 * @param {string} repoRoot Absolute path to the deck-machine repo root
 */
